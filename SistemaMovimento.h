...
private:
  ...
  Veicolo*     m_pTargetAgent1;
  Veicolo*     m_pTargetAgent2;
  ...
  Vector2D Pursuit(const Veicolo* agent);

  //this behavior maintains a position, in the direction of offset
  //from the target vehicle
  Vector2D OffsetPursuit(const Veicolo* agent, const Vector2D offset);

  //this behavior attempts to evade a pursuer
  Vector2D Evade(const Veicolo* agent);
  ...
  Vector2D Interpose(const Veicolo* VehicleA, const Veicolo* VehicleB);

  //given another agent position to hide from and a list of BaseGameEntitys this
  //method attempts to put an obstacle between itself and its opponent
  Vector2D Hide(const Veicolo* hunter, const std::vector<BaseGameEntity*>& obstacles);


  // -- Group Behaviors -- //

  Vector2D Cohesion(const std::vector<Veicolo*> &agents);
  
  Vector2D Separation(const std::vector<Veicolo*> &agents);

  Vector2D Alignment(const std::vector<Veicolo*> &agents);

  //the following three are the same as above but they use cell-space
  //partitioning to find the neighbors
  Vector2D CohesionPlus(const std::vector<Veicolo*> &agents);
  Vector2D SeparationPlus(const std::vector<Veicolo*> &agents);
  Vector2D AlignmentPlus(const std::vector<Veicolo*> &agents);
...
public:
  ...
  SteeringBehavior(Veicolo* agent);
  ...
  void      SetTargetAgent1(Veicolo* Agent){m_pTargetAgent1 = Agent;}
  void      SetTargetAgent2(Veicolo* Agent){m_pTargetAgent2 = Agent;}

  void      SetOffset(const Vector2D offset){m_vOffset = offset;}
  Vector2D  GetOffset()const{return m_vOffset;}

  //SetPath utilizza la funzione Set della Classe Path per istanziare un nuovo Path 
  void      SetPath(std::list<Vector2D> new_path){m_pPath->Set(new_path);}
  void      CreatePathNormale(/*int num_waypoints, int mx, int my, int cx, int cy*/int x, int y)const
            {m_pPath->CreatePathNormale(/*num_waypoints, mx, my, cx, cy*/ x , y);}
  ...
  void PursuitOn(Veicolo* v){m_iFlags |= pursuit; m_pTargetAgent1 = v;}
  void EvadeOn(Veicolo* v){m_iFlags |= evade; m_pTargetAgent1 = v;}
  ...
  void InterposeOn(Veicolo* v1, Veicolo* v2){m_iFlags |= interpose; m_pTargetAgent1 = v1; m_pTargetAgent2 = v2;}
  void HideOn(Veicolo* v){m_iFlags |= hide; m_pTargetAgent1 = v;}
  void OffsetPursuitOn(Veicolo* v1, const Vector2D offset){m_iFlags |= offset_pursuit; m_vOffset = offset; m_pTargetAgent1 = v1;}  
  ...
  double PesoSeparazione ()const{return m_dWeightSeparation;}
  double PesoAllineamento()const{return m_dWeightAlignment;}
