...
//
//------------------------------------------------------------------------
Raven_Steering::Raven_Steering(MondoDiGioco* world, Veicolo* agent):
             ...
             m_dViewDistance(1.0),
             m_dWallDetectionFeelerLength(25.0 * 3.0),
             m_Feelers(3),
...
{       
  ...
  //also a good behavior to add into this mix)
    ...
    if (On(separation))
    {
      force = Separation(m_pWorld->GetAllVeicoli()) * m_dWeightSeparation;
...
//------------------------------------------------------------------------
Vector2D Raven_Steering::Separation(const std::vector<Veicolo*>& neighbors)
{  
  //iterate through all the neighbors and calculate the vector from the
  Vector2D SteeringForce;

  std::vector<Veicolo*>::const_iterator it = neighbors.begin();
