...
void SistemaArmi::Initialize()
{
  ...
  m_pCurrentWeapon = new ArmaLeggera(m_pOwner);
...
//-----------------------------------------------------------------------------
void  SistemaArmi::AddWeapon(unsigned int weapon_type)
{
...
//-----------------------------------------------------------------------------
Raven_Weapon* SistemaArmi::GetWeaponFromInventory(int weapon_type)
{
  return m_WeaponMap[weapon_type];
}

//----------------------- ChangeWeapon ----------------------------------------
void SistemaArmi::ChangeWeapon(unsigned int type)
{
...
//-----------------------------------------------------------------------------
void SistemaArmi::TakeAimAndShoot()const
{
       ...
       m_dAimPersistance) )
  {
        ...
        GetCurrentWeapon()->GetType() == type_blaster)
    {
      ...
      if ( m_pOwner->RotateFacingTowardPosition(AimingPos)&&
		   ...
		   m_pOwner->m_iHealth < 5 )
      {
        AddNoiseToAim(AimingPos);

        GetCurrentWeapon()->Spara(AimingPos);
      }
    }
...
//  rads
//-----------------------------------------------------------------------------
void SistemaArmi::AddNoiseToAim(Vector2D& AimingPos)const
{
...
//  behavior.
//-----------------------------------------------------------------------------
Vector2D SistemaArmi::PredictFuturePositionOfTarget()const
{
...
//-----------------------------------------------------------------------------
int SistemaArmi::GetAmmoRemainingForWeapon(unsigned int weapon_type)
{
...
//-----------------------------------------------------------------------------
void SistemaArmi::Spara(Vector2D pos)const
{
  GetCurrentWeapon()->Spara(pos);
}

//-------------------------- RenderCurrentWeapon ------------------------------
//-----------------------------------------------------------------------------
void SistemaArmi::RenderCurrentWeapon()const
{
...
//-----------------------------------------------------------------------------
void SistemaArmi::RenderDesirabilities()const
{
