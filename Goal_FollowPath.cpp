...
//-----------------------------------------------------------------------------
Goal_FollowPath::
Goal_FollowPath(Veicolo*          pVeicolo,
                std::list<PathEdge> path):Goal_Composite<Veicolo>(pVeicolo, goal_follow_path),
                                                  m_Path(path)
{
}
...
{ 
  ...
  {  
	...
	gdi->TextAtPos(it->Destination().x,it->Destination().y,ttos(it->Destination()));
  }

  //forward the request to the subgoals
  Goal_Composite<Veicolo>::Render();
}
