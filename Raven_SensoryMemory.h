...
private:
  ...
  typedef std::map<Veicolo*, MemoryRecord> MemoryMap;
...
private:
  ...
  void       MakeNewRecordIfNotAlreadyPresent(Veicolo* pVeicolo);

public:

  Raven_SensoryMemory(Veicolo* owner, double MemorySpan);

  //this method is used to update the memory map whenever an opponent makes
  //a noise
  void     UpdateWithSoundSource(Veicolo* pNoiseMaker);

  //this removes a bot's record from memory
  void     RemoveBotFromMemory(Veicolo* pBot);
  ...
  bool     isOpponentShootable(Veicolo* pOpponent)const;
  bool     isOpponentWithinFOV(Veicolo* pOpponent)const;
  Vector2D GetLastRecordedPositionOfOpponent(Veicolo* pOpponent)const;
  double    GetTimeOpponentHasBeenVisible(Veicolo* pOpponent)const;
  double    GetTimeSinceLastSensed(Veicolo* pOpponent)const;
  double    GetTimeOpponentHasBeenOutOfView(Veicolo* pOpponent)const;

  //this method returns a list of all the opponents that have had their
  //records updated within the last m_dMemorySpan seconds.
  std::vector<Veicolo*> GetListOfRecentlySensedOpponents()const;
