...
//
//------------------------------------------------------------------------
SteeringBehavior::SteeringBehavior(Veicolo* agent):
                                  
             
             m_pVehicle(agent),
             m_iFlags(0),
             m_dDBoxLength(Prm.MinLunghezzaBoxDetenzione),
             m_dWeightCohesion(Prm.PesoCoesione),
             m_dWeightAlignment(Prm.PesoAllineamento),
             m_dWeightSeparation(Prm.PesoSeparazione ),
             m_dWeightObstacleAvoidance(Prm.PesoEvitareOstacoli),
             m_dWeightWander(Prm.PesoVagabondare),
             m_dWeightWallAvoidance(Prm.PesoEvitareMuri),
             m_dViewDistance(Prm.DistanzaVisuale),
             m_dWallDetectionFeelerLength(Prm.LunghezzaAntennaRiconoscimentoMuri),
             m_Feelers(3),
             ...
             m_dWeightSeek(Prm.PesoRicerca),
             m_dWeightFlee(Prm.PesoEvasione),
             m_dWeightArrive(Prm.PesoArrivo),
             m_dWeightPursuit(Prm.PesoInseguimento),
             m_dWeightOffsetPursuit(Prm.OffsetPesoInseguimento),
             m_dWeightInterpose(Prm.PesoInterposizione),
             m_dWeightHide(Prm.PesoNascondere),
             m_dWeightEvade(Prm.PesoEvadere),
             m_dWeightFollowPath(Prm.PesoSeguirePath),
...
{  
  ...
  if (On(wall_avoidance) && RandFloat() < Prm.prEvitareMuri)
  {
    m_vSteeringForce = WallAvoidance(m_pVehicle->World()->Walls()) *
                         m_dWeightWallAvoidance / Prm.prEvitareMuri;
  ...
  if (On(obstacle_avoidance) && RandFloat() < Prm.prEvitareOstacoli)
  {
    m_vSteeringForce += ObstacleAvoidance(m_pVehicle->World()->Obstacles()) * 
            m_dWeightObstacleAvoidance / Prm.prEvitareOstacoli;
  ...
  if (!isSpacePartitioningOn())
  {
    if (On(separation) && RandFloat() < Prm.prSeparazione)
    {
      m_vSteeringForce += Separation(m_pVehicle->World()->Agents()) * 
                          m_dWeightSeparation / Prm.prSeparazione;
  ...
  if (On(flee) && RandFloat() < Prm.prAllontanarsi)
  {
    m_vSteeringForce += Flee(m_pVehicle->World()->Crosshair()) * m_dWeightFlee / Prm.prAllontanarsi;
  ...
  if (On(evade) && RandFloat() < Prm.prEvasione)
  {
    assert(m_pTargetAgent1 && "Evade target not assigned");
    
    m_vSteeringForce += Evade(m_pTargetAgent1) * m_dWeightEvade / Prm.prEvasione;
  ...
  if (!isSpacePartitioningOn())
  {
    if (On(allignment) && RandFloat() < Prm.prAllineamento)
    {
      m_vSteeringForce += Alignment(m_pVehicle->World()->Agents()) *
                          m_dWeightAlignment / Prm.prAllineamento;
    ...
    if (On(cohesion) && RandFloat() < Prm.prCoesione)
    {
      m_vSteeringForce += Cohesion(m_pVehicle->World()->Agents()) * 
                          m_dWeightCohesion / Prm.prCoesione;
  ...
  if (On(wander) && RandFloat() < Prm.prVagabondaggio)
  {
    m_vSteeringForce += Wander() * m_dWeightWander / Prm.prVagabondaggio;
  ...
  if (On(seek) && RandFloat() < Prm.prRicerca)
  {
    m_vSteeringForce += Seek(m_pVehicle->World()->Crosshair()) * m_dWeightSeek / Prm.prRicerca;
...
//  evader
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Pursuit(const Veicolo* evader)
{
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Evade(const Veicolo* pursuer)
{
...
Vector2D SteeringBehavior::ObstacleAvoidance(const std::vector<BaseGameEntity*>& obstacles)
{
  ...
  if (ClosestIntersectingObstacle)
  {
    ...
    ForzaMovimento .y = (ClosestIntersectingObstacle->BRadius()-
    ...
    ForzaMovimento .x = (ClosestIntersectingObstacle->BRadius() - 
                       LocalPosOfClosestObstacle.x) * 
                       BrakingWeight;
  }

  //finally, convert the steering vector from local to world space
  return VectorToWorldSpace(ForzaMovimento ,
                            m_pVehicle->Heading(),
                            m_pVehicle->Side());
}
...
Vector2D SteeringBehavior::WallAvoidance(const std::vector<Wall2D>& walls)
{
  ...
  for (unsigned int flr=0; flr<m_Feelers.size(); ++flr)
  {
    ...
    if (ClosestWall >=0)
    {
      ...
      ForzaMovimento  = walls[ClosestWall].Normal() * OverShoot.Length();
    }

  }//next feeler

  return ForzaMovimento ;
}
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Separation(const vector<Veicolo*> &neighbors)
{  
  Vector2D ForzaMovimento ;
  ...
  for (unsigned int a=0; a<neighbors.size(); ++a)
  {
      ...
      (neighbors[a] != m_pTargetAgent1))
    {
      ...
      //from its neighbor.
      ForzaMovimento  += Vec2DNormalize(ToAgent)/ToAgent.Length();
    }
  }
...
//  of its neighbors
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Alignment(const vector<Veicolo*>& neighbors)
{
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Cohesion(const vector<Veicolo*> &neighbors)
{
  //first find the center of mass of all the agents
  Vector2D CenterOfMass, ForzaMovimento ;
  ...
  if (NeighborCount > 0)
  {
    ...
    ForzaMovimento  = Seek(CenterOfMass);
  }

  //the magnitude of cohesion is usually much larger than separation or
  //allignment so it usually helps to normalize it.
  return Vec2DNormalize(ForzaMovimento );
}
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::SeparationPlus(const vector<Veicolo*> &neighbors)
{  
  Vector2D ForzaMovimento ;
...
//  USES SPACIAL PARTITIONING
//------------------------------------------------------------------------
Vector2D SteeringBehavior::AlignmentPlus(const vector<Veicolo*> &neighbors)
{
...
//  USES SPACIAL PARTITIONING
//------------------------------------------------------------------------
Vector2D SteeringBehavior::CohesionPlus(const vector<Veicolo*> &neighbors)
{
  //first find the center of mass of all the agents
  Vector2D CenterOfMass, ForzaMovimento ;
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Interpose(const Veicolo* AgentA,
                                     const Veicolo* AgentB)
{
...
//
//------------------------------------------------------------------------
Vector2D SteeringBehavior::Hide(const Veicolo*           hunter,
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::OffsetPursuit(const Veicolo*  leader,
...
{ 
  ...
  if (m_pVehicle->ID() == 0){ gdi->TextAtPos(5,NextSlot,"MaxForce(Ins/Del):"); gdi->TextAtPos(160,NextSlot,ttos(m_pVehicle->MaxForce()/Prm.MoltiplicatoreForzaMovimento )); NextSlot+=SlotSize;}
  ...
  {  
    gdi->RedPen();
    Vector2D F = (m_vSteeringForce / Prm.MoltiplicatoreForzaMovimento ) * Prm.ScalaVeicolo ;
  ...
  if (m_pVehicle->World()->RenderDetectionBox())
  {
    ...
    double length = Prm.MinLunghezzaBoxDetenzione + 
                  (m_pVehicle->Speed()/m_pVehicle->MaxSpeed()) *
                  Prm.MinLunghezzaBoxDetenzione;
...
/////////////////////////////////////////////////////
  ...
  if (On(separation))
  {
    if (m_pVehicle->ID() == 0){ gdi->TextAtPos(5, NextSlot, "Separation(S/X):");gdi->TextAtPos(160,NextSlot, ttos(m_dWeightSeparation/Prm.MoltiplicatoreForzaMovimento ));NextSlot+=SlotSize;}

    if (KEYDOWN('S')){m_dWeightSeparation += 200*m_pVehicle->TimeElapsed(); Clamp(m_dWeightSeparation, 0.0f, 50.0f * Prm.MoltiplicatoreForzaMovimento );}
    if (KEYDOWN('X')){m_dWeightSeparation -= 200*m_pVehicle->TimeElapsed();Clamp(m_dWeightSeparation, 0.0f, 50.0f * Prm.MoltiplicatoreForzaMovimento );}
  }

  if (On(allignment))
  {
    if (m_pVehicle->ID() == 0) {gdi->TextAtPos(5, NextSlot, "Alignment(A/Z):"); gdi->TextAtPos(160, NextSlot, ttos(m_dWeightAlignment/Prm.MoltiplicatoreForzaMovimento ));NextSlot+=SlotSize;}

    if (KEYDOWN('A')){m_dWeightAlignment += 200*m_pVehicle->TimeElapsed();Clamp(m_dWeightAlignment, 0.0f, 50.0f * Prm.MoltiplicatoreForzaMovimento );}
    if (KEYDOWN('Z')){m_dWeightAlignment -= 200*m_pVehicle->TimeElapsed();Clamp(m_dWeightAlignment, 0.0f, 50.0f * Prm.MoltiplicatoreForzaMovimento );}
  }

  if (On(cohesion))
  {
    if (m_pVehicle->ID() == 0) {gdi->TextAtPos(5, NextSlot, "Cohesion(D/C):"); gdi->TextAtPos(160, NextSlot, ttos(m_dWeightCohesion/Prm.MoltiplicatoreForzaMovimento ));NextSlot+=SlotSize;}
    if (KEYDOWN('D')){m_dWeightCohesion += 200*m_pVehicle->TimeElapsed();Clamp(m_dWeightCohesion, 0.0f, 50.0f * Prm.MoltiplicatoreForzaMovimento );}
    if (KEYDOWN('C')){m_dWeightCohesion -= 200*m_pVehicle->TimeElapsed();Clamp(m_dWeightCohesion, 0.0f, 50.0f * Prm.MoltiplicatoreForzaMovimento );}
  }
