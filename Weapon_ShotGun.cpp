...
//--------------------------- ctor --------------------------------------------
//-----------------------------------------------------------------------------
ShotGun::ShotGun(Veicolo*   owner):

                      Raven_Weapon(type_shotgun,
                                   15,//script->GetInt("ShotGun_DefaultRounds"),
                                   50,//script->GetInt("ShotGun_MaxRoundsCarried"),
                                   1,//script->GetDouble("ShotGun_FiringFreq"),
                                   100,//script->GetDouble("ShotGun_IdealRange"),
                                   5000,//script->GetDouble("Pellet_MaxSpeed"),
                                   owner),

            m_iNumBallsInShell(10),//script->GetInt("ShotGun_NumBallsInShell")),
            m_dSpread(0.05)//script->GetDouble("ShotGun_Spread"))
{
...
inline void ShotGun::Spara(Vector2D pos)
{ 
  if (NumRoundsRemaining() > 0 && isReadyForNextShot())
  {
    ...
    m_pOwner->GetWorld()->GetMap()->AddSoundTrigger(m_pOwner,400);// script->GetDouble("ShotGun_SoundRange"));
  }
}
